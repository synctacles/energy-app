...
def normalize_prices(country: str = "NL"):
    ...
    try:
        ...
        _LOGGER.info(f"Price normalizer completed: {normalized} records normalized in {elapsed:.2f}s")

    except Exception as err:
        session.rollback()
        elapsed = time.time() - start_time
        _LOGGER.error(f"Price normalizer failed after {elapsed:.2f}s: {type(err).__name__}: {err}")
        raise
    finally:
        session.close()
