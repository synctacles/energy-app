...
def get_timer_status(timer_name: str) -> dict:
    ...
    result = subprocess.run(
        ["systemctl", "is-active", full_name], capture_output=True, text=True
    )
...
def pipeline_metrics(db: Session = Depends(get_db)):
    ...
    for timer_name, status in timers.items():
        ...
        if status["last_trigger_ago_min"] is not None:
            timer_last_trigger_minutes.labels(timer=timer_name).set(
                status["last_trigger_ago_min"]
            )
    ...
    return Response(
        content=generate_latest(pipeline_registry), media_type=CONTENT_TYPE_LATEST
    )
