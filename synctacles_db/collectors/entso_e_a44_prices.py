...
def main():
    ...
    try:
        ...
        _LOGGER.info(f"Fetching A44 prices for TOMORROW: {start_tomorrow} to {end_tomorrow}")
        ...
        elapsed = time.time() - start_time
        _LOGGER.error(f"A44 collector failed after {elapsed:.2f}s: {type(err).__name__}: {err}")
        raise
