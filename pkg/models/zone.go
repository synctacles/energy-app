...
func (r *ZoneRegistry) DetectZone(lat, lon float64, timezone, haCountry string) *ZoneDetectResult {
	// 0. If HA country is set and not in our registry, this is an unsupported region.
	// Don't match on coordinates alone — e.g. Morocco should not match Ceuta (ES).
	if haCountry != "" {
		_, inCountries := r.countries[haCountry]
		_, hasAlias := r.aliases[haCountry]
		if !inCountries && !hasAlias {
			return nil
		}
	}
