...
// ADR_016: When priceLookup is available, use pre-computed consumer price for wholesale→consumer.
func (n *Normalizer) normalizeAuto(snap normalizerSnapshot, p models.HourlyPrice) models.HourlyPrice {
	...
	if p.IsConsumer {
		// Consumer prices already include taxes — sensor is the truth, don't override.
	...
	// Wholesale price → convert to consumer.
	// ADR_016: prefer pre-computed consumer price from Worker (exact, includes markup + tax).
