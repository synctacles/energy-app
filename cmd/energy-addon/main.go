...
func main() {
	...
	// Non-wholesale zones (fixed/TOU) skip this — their tariffs are embedded in the YAML.
	// For wholesale zones: retry up to 3 times if Worker is unreachable.
	earlyHasWholesale := true
	if z, ok := registry.GetZone(cfg.BiddingZone); ok {
		earlyHasWholesale = z.HasWholesale()
	}
	isRegulatedMode := !earlyHasWholesale && (cfg.PricingMode == config.ModeFixed || cfg.PricingMode == config.ModeTOU)
	if !taxCache.HasData() && isRegulatedMode {
		slog.Info("regulated zone — Worker tax profile not needed", "zone", cfg.BiddingZone, "mode", cfg.PricingMode)
	} else if !taxCache.HasData() {
	...
	// Set currency from zone's country config (overrides ENV default)
	if cc, ok := registry.GetCountryForZone(cfg.BiddingZone); ok && cc.Currency != "" {
		cfg.Currency = cc.Currency
	}
	...
	if !zoneHasWholesale && (cfg.PricingMode == config.ModeFixed || cfg.PricingMode == config.ModeTOU) {
		...
		// Fixed mode or TOU parse failed: flat 24h at the regulated/configured rate
		// Prefer zone registry rate (correct for current zone) over cfg (may be stale from previous zone)
		fixedRate := cfg.FixedRatePrice
		if z, ok := registry.GetZone(cfg.BiddingZone); ok && z.RegulatedTariffs != nil && z.RegulatedTariffs.Fixed > 0 {
