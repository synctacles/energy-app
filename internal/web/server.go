...
func NewServer(deps Deps) *Server {
	...
	r.Route("/api", func(r chi.Router) {
		...
		r.Post("/fix-location", s.handleFixLocation)
...
func (s *Server) handleDashboard(w http.ResponseWriter, r *http.Request) {
	...
	dashboard["currency"] = s.cfg.Currency
	...
	isRegulated := s.cfg.PricingMode == "fixed" || s.cfg.PricingMode == "tou"
	if s.cfg.IsExternalSensorMode() {
		dashboard["price_accuracy"] = "exact"
	} else if isRegulated {
		dashboard["price_accuracy"] = "regulated"
...
func (s *Server) handleConfigSave(w http.ResponseWriter, r *http.Request) {
	...
	// Track zone/mode changes for hot-swap (no restart needed)
	zoneChanged := false
	...
	// Hot-swap zone and/or pricing mode without restart
	if (zoneChanged || modeChanged) && s.scheduler != nil {
		newZone := s.cfg.BiddingZone
		newMode := s.cfg.PricingMode
		hasWholesale := true
		var zoneLoc *time.Location

		if s.zoneRegistry != nil {
			if z, ok := s.zoneRegistry.GetZone(newZone); ok {
				hasWholesale = z.HasWholesale()
				if loc, err := time.LoadLocation(z.Timezone); err == nil {
					zoneLoc = loc
				}
			}
			// Update currency from country config (mutex-protected)
			if cc, ok := s.zoneRegistry.GetCountryForZone(newZone); ok && cc.Currency != "" {
				s.cfgMu.Lock()
				s.cfg.Currency = cc.Currency
				s.cfgMu.Unlock()
			}
		}
		if zoneLoc == nil {
		...
		s.scheduler.SetZoneInfo(hasWholesale, newMode, zoneLoc)
		if modeChanged && s.normalizer != nil {
			s.normalizer.SetPricingMode(newMode)
		}

		// For non-wholesale fixed/TOU zones: seed dashboard with synthetic prices
		if !hasWholesale && (newMode == "fixed" || newMode == "tou") {
			s.seedRegulatedPrices(newZone, newMode, zoneLoc)
		} else {
			s.scheduler.TriggerFetch()
		}
		slog.Info("hot-swapped zone/mode", "zone", newZone, "mode", newMode, "wholesale", hasWholesale)
	}

	writeJSON(w, map[string]string{"status": "saved", "message": "Settings saved."})
}

// seedRegulatedPrices generates synthetic prices for non-wholesale zones (fixed/TOU)
// and updates the sensor data. Called after hot-swap to fixed or TOU mode.
// Reads tariffs from the zone registry (not cfg) to ensure correct prices after zone change.
func (s *Server) seedRegulatedPrices(zone, mode string, zoneLoc *time.Location) {
	now := time.Now().UTC()
	var todayPrices []models.HourlyPrice

	// Get regulated tariffs from zone registry (source of truth for the zone)
	var fixedRate float64
	if s.zoneRegistry != nil {
		if z, ok := s.zoneRegistry.GetZone(zone); ok && z.RegulatedTariffs != nil {
			fixedRate = z.RegulatedTariffs.Fixed
		}
	}
	// Fallback to config if zone registry doesn't have it
	if fixedRate == 0 {
		fixedRate = s.cfg.FixedRatePrice
	}

	// TOU: try config TOU JSON first, then fall back to zone registry presets
	if mode == "tou" {
		var touParsed bool
		if s.cfg.TOUConfigJSON != "" {
			if touCfg, err := engine.ParseTOUConfig(s.cfg.TOUConfigJSON); err == nil {
				today, tomorrow := engine.GenerateTOUPrices(touCfg, zoneLoc)
				for i := range today {
					today[i].Zone = zone
				}
				for i := range tomorrow {
					tomorrow[i].Zone = zone
				}
				todayPrices = append(today, tomorrow...)
				touParsed = true
			}
		}
		// Fallback: build TOU from zone registry presets + regulated rates
		if !touParsed && s.zoneRegistry != nil {
			presets := s.zoneRegistry.GetTOUPresets(zone)
			if z, ok := s.zoneRegistry.GetZone(zone); ok && z.RegulatedTariffs != nil && len(presets) > 0 {
				preset := presets[0]
				rates := z.RegulatedTariffs.TOU[preset.ID]
				touJSON, _ := json.Marshal(map[string]any{
					"preset_id": preset.ID, "preset_name": preset.Name,
					"periods": preset.Periods,
					"rates":   map[string]float64{"peak": rates.Peak, "offpeak": rates.Offpeak},
				})
				if touCfg, err := engine.ParseTOUConfig(string(touJSON)); err == nil {
					today, tomorrow := engine.GenerateTOUPrices(touCfg, zoneLoc)
					for i := range today {
						today[i].Zone = zone
					}
	...
	// Fixed mode or TOU parse failed: flat 24h at the regulated rate
	if len(todayPrices) == 0 && fixedRate > 0 {
		...
		for h := 0; h < 24; h++ {
			...
			todayPrices = append(todayPrices, models.HourlyPrice{
				Timestamp: ts.UTC(), PriceEUR: fixedRate,
				Unit: models.UnitKWh, Source: "fixed", Quality: "live",
				Zone: zone, IsConsumer: true,
			})
		}
	}
	...
	if len(todayPrices) > 0 {
		...
		ss := hasensor.ComputeSensorSet(
			zone, todayPrices, nil,
			nil, fetchResult, now, "", s.cfg.BestWindowHours, mode,
		)
		ss.Source = "regulated"
		ss.Quality = "static"
		s.sensorData.Update(ss)
		slog.Info("seeded dashboard with regulated tariff", "zone", zone, "mode", mode, "price", ss.CurrentPrice)
	}
}
...
func (s *Server) handleZoneDetect(w http.ResponseWriter, r *http.Request) {
	...
	// Suggest timezone/country corrections when HA config doesn't match detected zone
	...
	// Currency correction based on zone's country
	...
	if len(corrections) > 0 {
		resp["location_corrections"] = corrections
	}

	writeJSON(w, resp)
}

// handleFixLocation applies suggested timezone/country corrections to HA core config.
// Called from the wizard when zone detection finds a mismatch.
func (s *Server) handleFixLocation(w http.ResponseWriter, r *http.Request) {
	if s.supervisor == nil {
		writeError(w, http.StatusServiceUnavailable, "not running inside HA")
		return
	}
	...
	// Validate inputs
	if req.Timezone != "" && (len(req.Timezone) > 50 || strings.ContainsAny(req.Timezone, "<>\"'&")) {
		writeError(w, http.StatusBadRequest, "invalid timezone")
		return
	}
	if req.Country != "" && (len(req.Country) > 5 || strings.ContainsAny(req.Country, "<>\"'&")) {
		writeError(w, http.StatusBadRequest, "invalid country")
		return
	}
	if req.Currency != "" && (len(req.Currency) > 5 || strings.ContainsAny(req.Currency, "<>\"'&")) {
	...
	if len(updates) == 0 {
		writeError(w, http.StatusBadRequest, "no corrections provided")
		return
	}

	if err := s.supervisor.UpdateCoreConfig(r.Context(), updates); err != nil {
		slog.Error("failed to update HA core config", "error", err)
		writeError(w, http.StatusInternalServerError, "failed to update HA configuration")
		return
	}

	slog.Info("HA location corrected", "updates", updates)
	writeJSON(w, map[string]any{"success": true, "applied": updates})
}
