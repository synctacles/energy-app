...
FILTERS = {
    "b16_solar_mw": lambda df: (df["timestamp"].dt.hour >= 8)
    & (df["timestamp"].dt.hour < 20),  # Alleen dag
...
for col in PSR_COLUMNS:
    ...
    if len(zeros) > 0:
        ...
        print(f"\n{psr_name} (abnormaal):")
        print(
            f"  Nullen: {len(zeros)} / {len(df_a75)} ({len(zeros) / len(df_a75) * 100:.1f}%)"
        )
        print("  Laatste 5:")
...
print("=" * 80)
total_abnormal = sum(
    len(df_a75[(df_a75[col] == 0.0) & (FILTERS.get(col, lambda x: x == x)(df_a75))])
    for col in PSR_COLUMNS
)
