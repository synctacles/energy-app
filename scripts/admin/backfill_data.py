...
class GapDetector:
    ...
    def _detect_a75_gaps(self, country: str) -> list[Gap]:
        ...
        for psr_type in psr_types:
            try:
                ...
                max_ts = (
                    self.session.query(func.max(RawEntsoeA75.timestamp))
                    .filter(
                ...
                if max_ts is None:
                    ...
                    self.logger.info(
                        f"A75 {psr_type}: No data, suggesting backfill from {max_ts}"
                    )
                ...
                if gap_seconds > 3600:  # Gap > 1 hour
                    gaps.append(
                        Gap(source="a75", category=psr_type, start=max_ts, end=now)
                    )
                    self.logger.info(
                        f"A75 {psr_type}: Gap detected ({gap_seconds / 3600:.1f} hours)"
                    )
    ...
    def _detect_a65_gaps(self, country: str) -> list[Gap]:
        ...
        for data_type in types:
            try:
                max_ts = (
                    self.session.query(func.max(RawEntsoeA65.timestamp))
                    .filter(
                        RawEntsoeA65.country == country, RawEntsoeA65.type == data_type
                    )
                    .scalar()
                )
                ...
                if gap_seconds > 3600:
                    gaps.append(
                        Gap(source="a65", category=data_type, start=max_ts, end=now)
                    )
                    self.logger.info(
                        f"A65 {data_type}: Gap detected ({gap_seconds / 3600:.1f} hours)"
                    )
    ...
    def _detect_a44_gaps(self, country: str) -> list[Gap]:
        ...
        try:
            max_ts = (
                self.session.query(func.max(RawEntsoeA44.timestamp))
                .filter(RawEntsoeA44.country == country)
                .scalar()
            )
            ...
            if gap_seconds > 3600:
                gaps.append(Gap(source="a44", category=None, start=max_ts, end=now))
    ...
    def _detect_tennet_gaps(self) -> list[Gap]:
        ...
        for platform in platforms:
            try:
                max_ts = (
                    self.session.query(func.max(RawTennetBalance.timestamp))
                    .filter(RawTennetBalance.platform == platform)
                    .scalar()
                )
                ...
                if gap_seconds > 300:  # Gap > 5 minutes for real-time data
                    gaps.append(
                        Gap(source="tennet", category=platform, start=max_ts, end=now)
                    )
                    self.logger.info(
                        f"TenneT {platform}: Gap detected ({gap_seconds / 60:.0f} minutes)"
                    )
...
class CollectionTrigger:
    ...
    def _trigger_a44(self, gap: Gap) -> CollectionResult:
        ...
        return self._run_collector(
            "sparkcrawler_entso_e_a44_prices.py", gap.start, gap.end
        )

    def _trigger_tennet(self, gap: Gap) -> CollectionResult:
        """Trigger TenneT collection"""
        return self._run_collector(
            "sparkcrawler_tennet_ingestor.py", gap.start, gap.end
        )

    def _trigger_energy_charts(self, gap: Gap) -> CollectionResult:
        """Trigger Energy-Charts collection"""
        return self._run_collector("energy_charts_prices.py", gap.start, gap.end)

    def _run_collector(
        self, script_name: str, start: datetime, end: datetime
    ) -> CollectionResult:
        ...
        if not script_path.exists():
            return CollectionResult(
                status="FAILED", stderr=f"Script not found: {script_path}"
            )
        ...
        try:
            ...
            duration = time.time() - start_time
            return CollectionResult(status="FAILED", stderr=str(e), duration=duration)
...
class BackfillLogger:
    ...
    def log_backfill(
        self, gap: Gap, result: CollectionResult, records_inserted: int = 0
    ):
...
Examples:
  ...
  python backfill_data.py --source all --max-gap-days 7
    ...
    parser.add_argument(
        "--source",
        choices=["a75", "a65", "a44", "tennet", "energy_charts", "all"],
        default="all",
        help="Data source to backfill",
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Detect gaps only, do not backfill"
    )
    parser.add_argument(
        "--max-gap-days",
        type=int,
        default=30,
        help="Maximum gap size to backfill (days)",
    )
        ...
        for source in sources:
            ...
            for gap in gaps:
                ...
                if gap_days > args.max_gap_days:
                    logger.warning(
                        f"Skipping: gap size ({gap_days}d) exceeds max ({args.max_gap_days}d)"
                    )
