...
echo "== SYNCTACLES loadtest environment setup =="
...
echo "[2/4] Writing Markdown report generator..."

cat << 'PYEOF' > "${BASE_DIR}/tools/generate_report_md.py"
#!/usr/bin/env python3
import json
import math
import argparse
...
chmod +x "${BASE_DIR}/tools/generate_report_md.py"

echo "[3/4] Cleaning obsolete directories (if any)..."
rm -rf "${BASE_DIR}/docs" || true

echo "[4/4] Done."
echo "Next steps:"
echo "  make solo"
echo "  make combined"
echo "  make soak DURATION=45m"
echo "  python3 tools/generate_report_md.py --results-dir results --out reports/loadtest_report.md"
