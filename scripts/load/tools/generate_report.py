...
def summary_row(name: str, s: dict):
    ...
    return [
        ...
        f"{reqs.get('rate', 'n/a'):.2f} req/s"
        if isinstance(reqs.get("rate"), (int, float))
        else "n/a",
...
def main():
    ...
    ap.add_argument(
        "--results-dir",
        default="results",
        help="Directory containing k6 --summary-export JSON files",
    )
    ...
    if capacities:
        cap_table = [
            ["Test", "Measured req/s", f"Users @ {hits_per_user_per_day} hits/day/user"]
        ]
        ...
        story.append(
            Paragraph(
                "No valid req/s metrics found to compute capacity.", styles["BodyText"]
            )
        )
